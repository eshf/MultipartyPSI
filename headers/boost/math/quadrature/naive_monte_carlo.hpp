...
public:
    naive_monte_carlo(const F& integrand,
                      ...
                      u64 seed = 0) noexcept : m_num_threads{threads}, m_seed{seed}, m_volume(1)
    {
        ...
        m_num_threads = (std::max)(m_num_threads, static_cast<u64>(1));
        m_thread_calls.reset(new std::atomic<u64>[threads]);
        m_thread_Ss.reset(new std::atomic<Real>[threads]);
        m_thread_averages.reset(new std::atomic<Real>[threads]);

        Real avg = 0;
        for (u64 i = 0; i < m_num_threads; ++i)
        {
            for (u64 j = 0; j < m_lbs.size(); ++j)
            {
   ...
   Real m_integrate()
   {
      ...
      do{
         ...
         do {
            ...
            for (u64 i = 0; i < m_num_threads; ++i)
            {
               u64 t_calls = m_thread_calls[i].load(std::memory_order_consume);
               total_calls += t_calls;
            }
            Real variance = 0;
            Real avg = 0;
    ...
    void m_thread_monte(u64 thread_index, u64 seed)
    {
        using std::numeric_limits;
        try
        {
            ...
            Real compensator = 0;
            u64 k = m_thread_calls[thread_index].load(std::memory_order_consume);
